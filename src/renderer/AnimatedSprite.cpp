...
namespace Renderer {
	...
	{
		m_pCurrentAnimateionDurations = m_statesMap.end();
	}
	...
	void AnimatedSprite::setState(const std::string& newState)
	{
		...
		if (it != m_pCurrentAnimateionDurations)
		{
			m_currentAnimationTime = 0;
			m_currentFrame = 0;
			m_pCurrentAnimateionDurations = it;
			m_dirty = true;
		}
	}
	...
	{
		if (m_pCurrentAnimateionDurations != m_statesMap.end())
		{
			m_currentAnimationTime += delta;

			while (m_currentAnimationTime >= m_pCurrentAnimateionDurations->second[m_currentFrame].second)
			{
				m_currentAnimationTime -= m_pCurrentAnimateionDurations->second[m_currentFrame].second;
				++m_currentFrame;
				m_dirty = true;
				if (m_currentFrame == m_pCurrentAnimateionDurations->second.size())
				{
					m_currentFrame = 0;
				}
			}
	...
	void AnimatedSprite::render() const
	{
		if (m_dirty) 
		{
			auto subTexture = m_pTexture->getSubTexture(std::move(m_pCurrentAnimateionDurations->second[m_currentFrame].first));
